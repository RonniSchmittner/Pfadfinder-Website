...
document.getElementById("datenschutz-link").onclick = function () {
  document.getElementById("datenschutz-modal").classList.remove("hidden");
};

document.getElementById("close-datenschutz").onclick = function () {
  document.getElementById("datenschutz-modal").classList.add("hidden");
};

document.getElementById("datenschutz-modal").onclick = function (e) {
  if (e.target === this) {
    this.classList.add("hidden");
  }
};
...
function updateParallax() {
  ...
  parallaxBgs.forEach((bg) => {
    ...
    if (rect.top < window.innerHeight && rect.bottom > 0) {
      const speed =
        (bg.getBoundingClientRect().height - rect.height) / window.innerHeight;
